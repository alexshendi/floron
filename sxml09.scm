...
(define *esc-table-1* 
  ;      0123456789012345678901234
        "&lt;&gt;&quot;&apos;&amp;")
(define *esc-table-2*
        (let ((tt (make-string 256)))
	  (let loop1 ((idx 0))
	    (if (>= idx 255)
	        #f
		(begin
		  (string-set! tt idx (integer->char 127))
		  (loop1 (+ idx 1)))))
	  (for-each (lambda (x p)
		      (string-set! tt (* (char->integer x) 2)
				   (integer->char (+ 1 (car p))))
		      (string-set! tt (+ 1 (* 2 (char->integer x)))
				   (integer->char (+ 1 (cadr p)))))
		    '(#\> #\< #\" #\' #\&)
		    '((0 4) (4 8) (8 14) (14 20) (20 25)))
	  tt))

(define (escape-char c)
  (let ((i1 (char->integer (string-ref *esc-table-2* (* 2 (char->integer c)))))
        (i2 (char->integer (string-ref *esc-table-2* (+ 1 (* 2 (char->integer c)))))))
    (if (= i1 127)
        #f
	(substring *esc-table-1* (- i1 1) (- i2 1))))) 
